...
class ProfileViewController: ButtonBarPagerTabStripViewController {
    ...
    func handleCommentsHistoryClick(comment : VSComment){
        ...
        if clickedComment?.parent_id == clickedComment?.post_id {
            performSegue(withIdentifier: "profileToChild", sender: self) //root comment was clicked, go to child page with this root comment as top card
        }
        else {
            //child page with clicked comment (child click) or clicked comment's parent (grandchild click) as top card
            performSegue(withIdentifier: "profileToGrandchild", sender: self)
        }
    }
