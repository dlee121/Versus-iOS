        ...
        for i in 0...list1.count-1 {
            ...
            while j < list2.count {
                ...
                if(value2.lowercased() < value1.lowercased()){
                    ...
                    combinedList.append(value1)
                    break
                }
                j += 1
            }
            
            if j >= list2.count { //if the while loop finished without the break statment getting called, then we need to append combinedList with the rest of list1
                for a in i...list1.count-1 {
                    combinedList.append(list1[a])
                }
                break
            }
        }
        if j < list2.count { //if the for loop finished without iterating completely through list2, then we need to append combinedList with the rest of list2
            for b in j...list2.count-1 {
                combinedList.append(list2[b])
            }
        }
